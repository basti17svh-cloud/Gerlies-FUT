...
test('existing one-team saves migrate with their XI, bench, reserve, formation and roles',()=>{
 ...
 assert.equal(state.squadPresets[0].tactic,'balanced');
...
test('new teams copy the active team once and later changes remain independent across reloads',async()=>{
 ...
 s.squad[0]='p18';s.squad[18]='p0';s.formation='4-2-3-1';s.tactic='attacking';s.roles[0]='Goalkeeper';s.focus[0]='Defend';
 ...
 assert.equal(s.tactic,'balanced');
 ...
 assert.equal(s.tactic,'attacking');
 ...
 assert.equal(recovered.squadPresets[1].tactic,'attacking');
